...
| `void set_led(LEDMode mode, LEDMask mask, uint8_t red, uint8_t green, uint8_t blue, uint16_t up_time, uint16_t down_time` | Set the `LEDMode` and `LEDMask`, the color for Red, Green and Blue and the up and down time for some modes.                            |
